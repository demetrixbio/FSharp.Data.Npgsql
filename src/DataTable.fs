...
type DataTable<'T when 'T :> DataRow>(selectCommand: NpgsqlCommand, ?connectionString: string, ?enumTypeColumns: string[]) = 
    inherit DataTable()
    ...
    member this.Update(?connection, ?transaction, ?batchSize, ?continueUpdateOnError, ?conflictOption) = 
        ...
        let isDirectTable = this.TableName <> null
        if selectCommand.Connection = null && isDirectTable
        then 
            assert(connectionString.IsSome)
            selectCommand.Connection <- new NpgsqlConnection( connectionString.Value)
